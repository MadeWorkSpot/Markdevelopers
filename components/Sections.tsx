...
export async function AboutSection() {
  ...
  return (
    <section className="relative bg-white px-4 py-24 md:px-8 lg:px-12 xl:px-24">
      ...
      <div className="mx-auto max-w-full">
        <div className="flex flex-col gap-12 lg:flex-row lg:items-center">
          <div className="lg:w-1/2">
            ...
            <p className="mt-8 text-lg leading-relaxed text-black/60 sm:text-xl">
            ...
            {(about.stats ?? []).length > 0 && (
              <div className="mt-10 grid grid-cols-2 gap-6 sm:grid-cols-4">
                {(about.stats ?? []).map((s) => (
                  <div key={s.label}>
                    <p className="text-3xl font-bold text-black sm:text-4xl">{s.number}</p>
            ...
            <Link
              href="/about"
              className="mt-10 inline-flex items-center gap-2 rounded-full border border-black px-8 py-3 text-sm font-medium uppercase tracking-wider text-black transition-all hover:bg-black hover:text-white"
            >
              {site.learnMoreLabel}
            </Link>
          </div>
...
export async function ProjectsSection() {
  ...
  return (
    <section className="relative bg-white px-4 py-24 md:px-8 lg:px-12 xl:px-24">
      <ProjectsCarousel
        projects={projects}
        heading={site.projectsSectionHeading}
        viewAllLabel={site.viewAllProjectsLabel}
      />
    </section>
  );
}
...
export async function ContactSection() {
  ...
  return (
    <section id="contact" className="bg-black px-4 py-24 md:px-8 lg:px-12 xl:px-24">
      <div className="mx-auto max-w-full">
        <div className="flex flex-col gap-4">
          <h2 className="text-4xl font-light leading-tight text-white sm:text-5xl">
            {contact.heading}
          </h2>
          <p className="mt-4 max-w-3xl text-lg leading-relaxed text-white/60 sm:text-xl">
            {contact.description}
          </p>
        </div>
        ...
        <div className="mt-14 grid gap-16 lg:grid-cols-2">
          <ContactForm
            nameLabel={site.contactNameLabel}
            namePlaceholder={site.contactNamePlaceholder}
            emailLabel={site.contactEmailLabel}
            emailPlaceholder={site.contactEmailPlaceholder}
            messageLabel={site.contactMessageLabel}
            messagePlaceholder={site.contactMessagePlaceholder}
            sendMessageLabel={site.sendMessageLabel}
          />

          <div className="space-y-12">
            <div>
              <h3 className="text-lg font-medium text-white">{site.contactDetailsHeading || "Contact Details"}</h3>
            ...
            <div>
              <h3 className="text-lg font-medium text-white">{site.officeHoursHeading || "Office Hours"}</h3>
