    ...
    <>
      <section className="relative flex min-h-[70vh] items-center overflow-hidden bg-black">
        ...
        <div className="relative z-10 px-4 md:px-8 lg:px-12 xl:px-24">
          <h1 className="mt-4 max-w-4xl text-5xl font-light leading-tight text-white sm:text-7xl">
      ...
      {about.companyStory?.content && (
        <section className="bg-white px-4 py-24 md:px-8 lg:px-12 xl:px-24">
          <div className="mx-auto max-w-full">
            <h2 className="text-4xl font-light leading-tight text-black sm:text-5xl">
              {about.companyStory.heading}
            </h2>
            <div className="mt-8 max-w-full text-lg leading-relaxed text-black/60 sm:text-xl whitespace-pre-line">
              {about.companyStory.content}
            </div>
          </div>
        </section>
      ...
      <section className="bg-black px-4 py-24 md:px-8 lg:px-12 xl:px-24">
        <div className="mx-auto max-w-full">
          <div className="grid gap-16 lg:grid-cols-2">
            {(about.values ?? []).map((v) => (
              <div key={v.title}>
                <span className="text-md font-medium uppercase tracking-[0.25em] text-white/50">
                  {v.title}
                </span>
                <p className="mt-4 leading-relaxed text-white/60 sm:text-lg">{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-black px-4 py-24 md:px-8 lg:px-12 xl:px-24">
        <div className="mx-auto max-w-full">
          ...
          <p className="mt-6 max-w-2xl leading-relaxed text-white/60 sm:text-lg">
            {labels.readyToStartDesc}
          </p>
          <Link
            href="/#contact"
            className="mt-10 inline-flex items-center gap-2 rounded-full border border-white px-8 py-3 text-sm font-medium uppercase tracking-wider text-white transition-all hover:bg-white hover:text-black"
          >
